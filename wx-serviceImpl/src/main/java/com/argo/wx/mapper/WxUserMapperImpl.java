 ...
 */
@Repository
public class WxUserMapperImpl extends MySqlMapper<WxUser, Integer> implements WxUserMapper {
    ...
    protected void setInsertStatementValues(PreparedStatement ps, WxUser item) throws SQLException {
        ...
        ps.setString(1, null == openId ? null : openId);

        String openToken = item.getOpenToken();
        ps.setString(2, null == openToken ? null : openToken);

        Date createTime = item.getCreateTime();
        ps.setDate(3, null == createTime ? null : new java.sql.Date(createTime.getTime()));

        Date bindTime = item.getBindTime();
        ps.setDate(4, null == bindTime ? null : new java.sql.Date(bindTime.getTime()));

        Integer userId = item.getUserId();
        ps.setInt(5, null == userId ? null : userId);

        Date lastAccessTime = item.getLastAccessTime();
        ps.setDate(6, null == lastAccessTime ? null : new java.sql.Date(lastAccessTime.getTime()));

        Byte statusId = item.getStatusId();
        ps.setByte(7, null == statusId ? 0 : statusId);

        Date rmBindTime = item.getRmBindTime();
        ps.setDate(8, null == rmBindTime ? null : new java.sql.Date(rmBindTime.getTime()));
