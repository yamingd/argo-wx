 ...
 */
@Repository
public class WxMenuMapperImpl extends MySqlMapper<WxMenu, Integer> implements WxMenuMapper {
    ...
    protected void setInsertStatementValues(PreparedStatement ps, WxMenu item) throws SQLException {
        ...
        ps.setString(1, null == title ? null : title);

        Integer parentId = item.getParentId();
        ps.setInt(2, null == parentId ? 0 : parentId);

        Short orderNo = item.getOrderNo();
        ps.setShort(3, null == orderNo ? 100 : orderNo);

        String codeName = item.getCodeName();
        ps.setString(4, null == codeName ? null : codeName);

        String pageUrl = item.getPageUrl();
        ps.setString(5, null == pageUrl ? null : pageUrl);

        Date createTime = item.getCreateTime();
        ps.setTimestamp(6, null == createTime ? null : new java.sql.Timestamp(createTime.getTime()));

        Date updateTime = item.getUpdateTime();
        ps.setTimestamp(7, null == updateTime ? null : new java.sql.Timestamp(updateTime.getTime()));

        Short statusId = item.getStatusId();
        ps.setShort(8, null == statusId ? 0 : statusId);

        Byte wxSync = item.getWxSync();
        ps.setByte(9, null == wxSync ? 0 : wxSync);

        Byte wxBind = item.getWxBind();
        ps.setByte(10, null == wxBind ? 0 : wxBind);
